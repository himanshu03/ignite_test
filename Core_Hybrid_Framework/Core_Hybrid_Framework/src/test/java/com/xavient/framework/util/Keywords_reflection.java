...
public class Keywords_reflection {
	...
	public static Xls_Reader xls1 = new Xls_Reader(
			Constants.SUITEWEBAPPLICATION_XLS_PATH);

	// public static Method capturescreenShot_method;

	public Keywords_reflection() throws NoSuchMethodException,
			SecurityException {
	...
	public void executeKeywords(String testName, Xls_Reader xls,
		...
		boolean b = xls.setCellData(Constants.KEYWORDS_SHEET, resutlcol, 1,
				"RESULT " + item);

		// boolean c = xls.setCellData(Constants.SCREENSHOTS_SHEET, resutlcol,
		// 1,"RESULT "+item);

		// System.out.println(".............." + item+ " "+resutlcol+" "+b +
		// rows);
		...
		for (int rNum = 2; rNum <= rows; rNum++) {
			...
			if (tcid.equalsIgnoreCase(testName)) {
				iteration_runmode = xls.getCellData(Constants.TestData_SHEET,
						1, rNum);
				String keyword = xls.getCellData(Constants.KEYWORDS_SHEET, 2,
						rNum);
				String object = xls.getCellData(Constants.KEYWORDS_SHEET, 3,
						rNum);
				String dataCol = xls.getCellData(Constants.KEYWORDS_SHEET, 4,
						rNum);
				log(keyword + " --- " + object + " --- " + dataCol);
				// System.out.println(".............." + item+ " "+resutlcol+"
				// "+b + iteration_runmode);
				// m=new Keywords_reflection();

				Method method[];
				method = Keywords_reflection.getInstance().getClass()
						.getMethods();
				for (int i = 0; i < method.length; i++) {
					// System.out.println(method[i].getName());
					if (method[i].getName().equals(keyword)) {
						int newValue = randomNumber(2);
						System.out.println("New Generated Value ::" + newValue);
						String result = (String) method[i].invoke(keyword,
								object, data.get(dataCol));
						// xls.setCellData(Constants.KEYWORDS_SHEET,resutlcol,
						// rNum,(String)method[i].invoke(keyword,object,data.get(dataCol)));
						xls.setCellData(Constants.KEYWORDS_SHEET, resutlcol,
								rNum, result);
					}
				}
		...
		try {
			...
			if (map.get(data.toLowerCase()) == null) {
				...
				if (data.equalsIgnoreCase(Constants.MOZILLA))
					...
					capabilities.setCapability("chrome.binary",prop.getProperty("chromedriverexe"));
					...
					System.setProperty("webdriver.ie.driver",
							prop.getProperty("IEdriverserverexe"));
	...
	public static String clickByJS(String Objectkey, String data) {
		log("Starting function click" + Objectkey);
		((JavascriptExecutor) driver).executeScript("arguments[0].click();",
				element(Objectkey, data));
		// element(Objectkey, data).click();
		log("Ending  function click with status " + Constants.PASS);
		return Constants.PASS;
	}

	public String verityText(String Objectkey, String data) {
		String actual = driver.findElement(
				By.xpath(prop.getProperty(Objectkey))).getAttribute("value");
	...
	public static String clear(String Objectkey, String data) {
		log("Starting function input ::" + Objectkey + " , " + data);
		element(Objectkey, data).clear();
		log("Ending  function click with status " + Constants.PASS);
		return Constants.PASS;
	}

	public static List<String> elements(String Locator) {
	...
	public static String verifyTitle(String Objectkey, String data)
			throws InterruptedException {// titles

		try {
			String actualTitle = driver.getTitle();
			String expectedTitle = data;
			if (!actualTitle.equals(expectedTitle)) {
				reportFailureAndStop(Constants.TITLE_NOT_MATCHES_FAILURE
	...
	public static String VerifyingField(String Objectkey, String data) {

		try {
			WebElement Locate = driver.findElement(By.xpath(prop.getProperty(Objectkey)));
		...
		try {
			if (Objectkey.endsWith("_id")) {
				...
				wait.until(ExpectedConditions.presenceOfElementLocated(By
						.id(prop.getProperty(Objectkey))));
				return driver.findElement(By.id(prop.getProperty(Objectkey)));
			} else if (Objectkey.endsWith("_name")) {
				WebDriverWait wait = new WebDriverWait(driver, 10);
				wait.until(ExpectedConditions.presenceOfElementLocated(By
						.name(prop.getProperty(Objectkey))));
				return driver.findElement(By.name(prop.getProperty(Objectkey)));
			} else if (Objectkey.endsWith("_xpath")) {
				WebDriverWait wait = new WebDriverWait(driver, 10);
				wait.until(ExpectedConditions.presenceOfElementLocated(By
						.xpath(prop.getProperty(Objectkey))));
				return driver
						.findElement(By.xpath(prop.getProperty(Objectkey)));
			} else if (Objectkey.endsWith("_partialLinkText")) {
				WebDriverWait wait = new WebDriverWait(driver, 10);
				wait.until(ExpectedConditions.presenceOfElementLocated(By
						.partialLinkText(prop.getProperty(Objectkey))));
				return driver.findElement(By.partialLinkText(prop
						.getProperty(Objectkey)));
			} else if (Objectkey.endsWith("_css")) {
				WebDriverWait wait = new WebDriverWait(driver, 10);
				wait.until(ExpectedConditions.presenceOfElementLocated(By
						.cssSelector(prop.getProperty(Objectkey))));
				return driver.findElement(By.cssSelector(prop
						.getProperty(Objectkey)));
			} else if (Objectkey.endsWith("_linkText")) {
				WebDriverWait wait = new WebDriverWait(driver, 10);
				wait.until(ExpectedConditions.presenceOfElementLocated(By
						.linkText(prop.getProperty(Objectkey))));
				return driver.findElement(By.linkText(prop
						.getProperty(Objectkey)));
			} else if (Objectkey.endsWith("_className")) {
				WebDriverWait wait = new WebDriverWait(driver, 10);
				wait.until(ExpectedConditions.presenceOfElementLocated(By
						.className(prop.getProperty(Objectkey))));
				return driver.findElement(By.className(prop
						.getProperty(Objectkey)));
			} else {// error
			...
			reportFailureAndStop(Constants.ELEMENT_NOT_FOUND_FAILURE
					+ Objectkey);
		} catch (Exception e) { // error
	...
	public static String verifyGoogleSearch(String ObjectKey, String data) {
		...
		try {
			...
			List<WebElement> findElements = driver.findElements(By.xpath(prop
					.getProperty(ObjectKey)));

			// this are all the links you like to visit
			for (WebElement webElement : findElements) {
				if (!webElement.getAttribute("href").toLowerCase()
						.contains(data.toLowerCase())) {
					reportFailureAndStop(Constants.DATA_NOT_MATCH_FAILURE
							+ data + " actual "
	...
	public static boolean isElementPresent(String objectKey, String timeout) {
		...
		int size = 0;
		if (objectKey.endsWith("_id"))
			size = driver.findElements(By.id(prop.getProperty(objectKey)))
					.size();
		else if (objectKey.endsWith("_name"))
			size = driver.findElements(By.name(prop.getProperty(objectKey)))
					.size();
		else if (objectKey.endsWith("_xpath"))
			size = driver.findElements(By.xpath(prop.getProperty(objectKey)))
					.size();
	...
	public static void waitForPageToLoad(String Objectkey, String data) {
		...
		while (!js.executeScript("return document.readyState").toString()
				.equals("complete")) {
			try {
				log("Waiting for 8 sec for page to load");
				Thread.sleep(8000);
	...
	public static Keywords_reflection getInstance()
	...
	public static void takeScreenShot() {
		...
		String filePath = Constants.SCREENSHOT_PATH + currentTestCaseName + "-"
				+ currentIteration + ".png";
		File targetFile = new File(filePath);
		File srcFile = ((TakesScreenshot) driver)
				.getScreenshotAs(OutputType.FILE);

		String SurefirereportFilePath = Constants.SUREFIREREPORTS
				+ currentTestCaseName + "-" + currentIteration + ".png";
		File Surefirereport = new File(SurefirereportFilePath);
		int iter = Integer.parseInt(currentIteration);
		xls1.addHyperLink(Constants.SCREENSHOTS_SHEET, "ScreenShots",
				currentTestCaseName.trim(), iter, filePath, "ITERATION " + iter);
